...
| 5 | **Bazzite is its own `$PM` (`bazzite`), not plain Fedora** | Atomic/immutable root fs (`/run/ostree-booted`, read-only) means `dnf install` would fail; detection gates on `ID=bazzite`/`VARIANT_ID=silverblue\|kinoite` or the ostree marker *before* the dnf branch, and the toolset installs via the image's Linuxbrew + shared Brewfile instead of a manifest, skipping `chsh` (Bazzite warns it can break session login). `bootstrap.sh` mirrors the same ostree/`ID` detection (`is_atomic_fedora`) before its package-manager branches, so git bootstrapping on an atomic image gets an rpm-ostree/toolbox pointer instead of a "no known package manager" dead end |
